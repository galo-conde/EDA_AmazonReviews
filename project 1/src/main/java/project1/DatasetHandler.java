...
public class DatasetHandler{
	...
	public void loadDB() throws IOException{
		...
		if(Files.notExists(dbPath)){
			...
			for (final String line : lines) {
				...
				System.out.println(col[2] + "   Path error");
	 ...
	 * @author conde
	 * @author padilla
	 * @returns string for data.csv file
	 */
	public String printDB(){
		...
		retVal.append("dataID,NUMBER,RATINGS,STATS\n");
			for (Dataset d  : db) {

				try {
					retVal.append(d.toString() + DataAnalysis.DELIMITER + defineStatPath(d.getDataId()) +// d.getratingsummaryfilepath 
								"\n");
	...
	/**
	 * implement writeDBToFile() method
	 * @author conde
	 * @author padilla
	 * @throws IOException data.csv is unable to written to
	 */
	public void writeDBToFile() throws IOException{
		
		//your code here
		
		FileWriter outputStream = new FileWriter(this.dbPath.toString());
		try {
			
			outputStream.write(this.printDB()); 
	...
	public void saveReportToFile(final String dataID, int k){
		...
		try{
			...
			String report = DataAnalysis.createReport(this.getCollection(dataID).getRatingStat(),k);
	...
	public void saveStatsToFile(final String dataID){
		...
		try{
			...
			StringBuilder summary = new StringBuilder(DataAnalysis.SUMMARY_HEADER + System.lineSeparator());
			summary.append(this.getCollection(dataID).saveStats());
			Files.writeString(this.defineStatPath(dataID),summary.toString());
